class_name SetLeftRightTargets, "../icons/btleaf.svg" 
extends BTLeaf


# Sets the targets for the left and right of target. Only works when card variable "hexagonal" is set to true or min and max range is 1.
export(int) var width: int = 1
export(int) var depth: int = 0
...
func _tick(agent: Node, blackboard: Blackboard) -> bool:
	...
	if startCell == null or startCell.get_class() != "HexCell":
		...
		var targets:Array = startCell.getLeftRight(agent,width,depth,splash_up_vertical_range,splash_down_vertical_range)
