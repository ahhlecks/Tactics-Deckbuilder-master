...
func loadNewCardData(data, level = 0) -> void:
#	card.load_card(data)
	card_info = data
	card_info.card_level = level
...
func updateCardInfo(unit:HexUnit = null) -> void:
	...
	if card_name.text.find("-"):
		var idx = card_name.text.find("-") + 1
		card_name.text = card_name.text.substr(idx,card_name.text.length())
	var item_types:String
	if !card_info.item.type.empty():
		for type in card_info.item.type:
			if type is String:
				if type != "none":
					item_types += type + " "
			else:
				if type != 0:
					item_types += str(BattleDictionary.item_type[type]) + " "
	card_type.text = item_types + card_types.get(card_info.card_type)
	
	setDescription(card_info)
	
	card_delay.text = str(round(float(card_info.delay[card_info.card_level])))
